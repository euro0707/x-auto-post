...
| B列 | 日にち | 日付（yyyy/mm/dd形式またはシリアル値） |
| C列 | 時 | 時刻（0〜23、空欄は0時） |
| D列 | 分 | 分（0〜59、空欄は0分） |
| E列 | 投稿URL | 投稿後のツイートURLまたはエラー情報 |
| F列 | スレッドグループID | スレッド投稿用のグループID（空欄は単独投稿） |
| G列 | ステータス | 投稿状態（pending/posting/posted/failed） |
...
1. A列に投稿本文を入力
2. B列に投稿日、C列に時、D列に分を入力
3. F列は空欄のまま（単独投稿）
4. トリガーまたは手動で`postNextScheduledToX()`を実行
5. 投稿成功時：E列にツイートURL、G列に`posted`
6. 文字数超過時：E列に「文字数超過: XX文字」、G列に`failed`
...
1. A列に各投稿の本文を入力（複数行）
2. B〜D列に最初の行の投稿日時を入力（他の行は空欄でOK）
3. **スレッドにしたい行を範囲選択**（2行以上）
4. **メニューバー「スレッド機能」→「選択行を1つのスレッドにまとめる」**
5. 自動的にF列に同じスレッドグループIDが設定される
6. トリガーまたは手動で`postNextScheduledToX()`を実行

**方法2: 手動でスレッドグループIDを入力**

1. A列に各投稿の本文を入力（複数行）
2. B〜D列に最初の行の投稿日時を入力（他の行は空欄でOK）
3. **F列に同じスレッドグループID**を入力（例: `thread001`）
4. トリガーまたは手動で`postNextScheduledToX()`を実行
5. 同じIDを持つすべての行が連続投稿される
6. 各行のE列に投稿URLが記録される

**スレッド投稿の例:**

| A列（本文） | B列 | C列 | D列 | E列 | F列 | G列 |
|------------|-----|-----|-----|-----|-----|-----|
| これは1つ目の投稿です。 | 2025/01/15 | 10 | 0 | | thread-20250130-001 | pending |
| これは2つ目の投稿です。 | | | | | thread-20250130-001 | pending |
| これは3つ目の投稿です。 | | | | | thread-20250130-001 | pending |

→ 実行すると、3つの投稿が連続してスレッド形式で投稿されます。
